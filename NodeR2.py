...
# Servicio que le permite a cada nodo proporcionar el segmento de un archivo. SERVIDOR CON UN ÚNICO SERVICIO.
...
#   clientTask > Funcionalidad del cliente
def clientTask():
        ...
        responseInitial = requests.post("http://192.168.191.100:5000/verifyPendingDownloads", json={"IP" : deviceIp})
                    ...
                    for i in range(peer["trackerSegment"], peer["LastFile"], 1): # Genera un bucle for para cada fragmento
                        ...
                        if response4.status_code == 200:
                            ...
                            try:
                                ...
                                response5 = requests.post("http://192.168.191.159:5000/updatePeers", json = {"fileName" : req["File2Download"], "IP" : deviceIp, "currentSegments": i, "numSegments": leechersAndSeeders[0]["numSegments"], "peerIP" : peer["IP"]})
                            except Exception as e:
        ...
        api_url = "http://192.168.191.159:5000/enterNetwork" # Se llama al servicio para incluir a este nodo en la red.
        try:
        ...
        while not exit:
            ...
            if select == "1":
                ...
                try:
                    response1 = requests.put( "http://192.168.191.159:5000/addFile/" + deviceIp, json=newFiles) # Llama al servicio para agregar un archivo a la red.
                ...
                try:
                    response2 = requests.get("http://192.168.191.159:5000/allFiles") # LLama al servicio para obtener todos los archivos disponibles de la red.
                    if response2.status_code == 200:
                        ...
                        try:
                            response3 = requests.post("http://192.168.191.159:5000/downloadFile", json={"fileName" : desiredFile, "IP" : deviceIp }) # LLama al servicio para descargar el archivo y le pasa la dirección IP del nodo y el archivo que se desea descargar.
                                   ...
                                   for peer in  leechersAndSeeders: # para cada peer
                                       for i in range(peer["StartingFile"], peer["LastFile"], 1): # Se genera un ciclo for para cada fragmento a descargar.
                                            ...
                                            if response4.status_code == 200:
                                                ...
                                                print(f"Archivo guardado en: {file_path}")
                                                try:
                                                    response5 = requests.post("http://192.168.191.159:5000/updatePeers", json = {"fileName" : desiredFile, "IP" : deviceIp, "currentSegments": i, "numSegments": leechersAndSeeders[0]["numSegments"], "peerIP" : peer["IP"]}) # Se le notifica al Tracker que fragmento  se ha recuperado y de que peer se ha recuperado.
                                                except Exception as e:
                                                    print(f"Ocurrió un error al solicitar un archivo: {e}")
                                                    import traceback
                                                    traceback.print_exc()
...
if __name__ == "__main__":
    ...
    # Corre el cliente -> Maneja el servidor.
    app.run(host="192.168.191.159", port=5001, debug=False, threaded=True)